...
detailEl.innerHTML = ` <div class="special-day-info"> <div class="special-icon">${ewig.isIntera ? "🌟" : "✨"}</div> <h3>${ewig.monthName}</h3> <p>${ewig.isIntera ? "Der Schalttag – nur alle 4 Jahre" : "Der zeitlose Tag – außerhalb aller Monate"}</p> <p class="special-desc">Im Ewigen Kalender steht ${ewig.monthName} am Ende des Jahres, nach Noctis (Monat 13). Er gehört zu keinem Monat und zu keiner Woche.</p> </div>`;
return;
}

const s = SEASONS[ewig.season];
mainEl.innerHTML = ` <div class="ewig-day-display ${seasonClass(ewig.season)}"> <span class="ewig-day-num">${ewig.day}</span> <div class="ewig-month-info"> <span class="ewig-month-name">${ewig.emoji} ${ewig.monthName}</span> <span class="ewig-month-sub">${ewig.monthSub}</span> </div> </div>`;
...
// Mini month calendar
detailEl.innerHTML = ` <div class="month-detail-inner"> <div class="month-detail-header ${seasonClass(ewig.season)}"> <span>${ewig.emoji} ${ewig.monthName}</span> <span class="month-detail-sub">${ewig.monthSub}</span> </div> <div class="month-detail-info"> <span>${weekday}, Woche ${week}</span> <span>Tag ${(ewig.month - 1) * 28 + ewig.day} des Jahres</span> </div> ${renderMiniMonth(ewig.month, ewig.day, ewig.season)} </div>`;
}

function renderMiniMonth(month, highlightDay, season) {
const m = MONTHS[month - 1];
let html = `<div class="mini-cal"> <div class="mini-cal-header">${WEEKDAYS.map(d => `<span>${d}</span>`).join("")}</div> <div class="mini-cal-body">`;
for (let d = 1; d <= 28; d++) {
...
unaraCard.innerHTML = ` <div class="year-month-title"><span>✨ Unara</span><span class="year-month-sub">Der zeitlose Tag</span></div> <div class="special-day-btn year-day" data-day="1" data-greg="${unaraDate.toISOString().split('T')[0]}" data-special="Unara"> <div class="special-day-circle">365</div> <div class="special-day-label">${formatGregShort(unaraDate)}</div> </div>`;
...
interaCard.innerHTML = ` <div class="year-month-title"><span>🌟 Intera</span><span class="year-month-sub">Der Schalttag</span></div> <div class="special-day-btn year-day" data-day="2" data-greg="${interaDate.toISOString().split('T')[0]}" data-special="Intera"> <div class="special-day-circle">366</div> <div class="special-day-label">${formatGregShort(interaDate)}</div> </div>`;
...
if (ewig.isUnara || ewig.isIntera) {
html += ` <div class="popup-special"> <div class="popup-icon">${ewig.isIntera ? "🌟" : "✨"}</div> <h3>${ewig.monthName}</h3> <div class="popup-greg">${formatGreg(date)}</div> <p>${ewig.isIntera ? "Schalttag · Alle 4 Jahre · Tag 366" : "Zeitloser Tag · Tag 365"}</p> </div>`;
} else {
const s = SEASONS[ewig.season];
const weekdayNames = [“Montag”,“Dienstag”,“Mittwoch”,“Donnerstag”,“Freitag”,“Samstag”,“Sonntag”];
const weekday = weekdayNames[(ewig.day - 1) % 7];
html += ` <div class="popup-ewig season-${ewig.season}"> <div class="popup-month-badge">${ewig.emoji} ${ewig.monthName}</div> <div class="popup-day-num">${ewig.day}</div> <div class="popup-month-sub">${ewig.monthSub}</div> </div> <div class="popup-details"> <div class="popup-detail-row"><span>📅 Gregorianisch</span><span>${formatGreg(date)}</span></div> <div class="popup-detail-row"><span>${s.emoji} Jahreszeit</span><span>${s.label}</span></div> <div class="popup-detail-row"><span>📆 Wochentag</span><span>${weekday}</span></div> <div class="popup-detail-row"><span>🗓 Monat</span><span>${ewig.month} von 13</span></div> <div class="popup-detail-row"><span>📊 Tag des Jahres</span><span>${(ewig.month-1)*28+ewig.day} von 364</span></div> </div>`;
}
